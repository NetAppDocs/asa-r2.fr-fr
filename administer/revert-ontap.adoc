...
NOTE: link:space-balancing.html["équilibrage dynamique de l'espace"]est activé par défaut 14 jours après la mise à niveau vers ONTAP 9.17.1 ou l'initialisation d'un nouveau cluster ONTAP 9.17.1 ASA r2.  Vous ne pouvez pas revenir à la version ONTAP 9.17.1 sur votre système ASA r2 après l'activation de l'équilibrage dynamique de l'espace.
...
| Relations d'importation actives | Supprimez les relations d'importation actives. link:../install-setup/set-up-data-access.html#migrate-data-from-a-third-party-storage-system["Découvrez les relations d'importation"]. 


| Protection anti-ransomware activée | link:../secure-data/pause-resume-anti-ransomware-protection.html["Suspendre la protection anti-ransomware"]. 
|===