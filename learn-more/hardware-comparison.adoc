...
Lors de la création d'une unité de stockage (basée sur un LUN ou un espace de noms NVMe), ONTAP crée automatiquement un volume dans la zone de disponibilité de stockage appropriée pour héberger cette unité. Le volume nouvellement créé est automatiquement placé dans la zone de disponibilité de stockage pour des performances optimales et une utilisation équilibrée de la capacité. L'utilisation de la capacité est équilibrée au sein de la zone de disponibilité de stockage en fonction de votre version d'ONTAP. link:../administer/space-balancing.html["Découvrez comment équilibrer la capacité dans un cluster ASA r2"^].
