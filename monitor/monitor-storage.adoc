...
Chaque paire haute disponibilité dans un système ASA r2 utilise un pool de stockage commun appelé . La zone de disponibilité de stockage a accès à tous les disques disponibles du système de stockage et est visible par les deux nœuds de la paire haute disponibilité. La surveillance de l'espace disponible dans vos zones de disponibilité de stockage vous permet de diminuer l'utilisation ou d'augmenter la capacité selon les besoins afin d'éviter des échecs de charges de travail dus à un espace insuffisant.
...
À partir d'ONTAP 9.19.1, une alerte s'affiche dans System Manager *Insights* si une zone de disponibilité de stockage dans l'une de vos paires haute disponibilité ASA r2 approche de sa capacité. En savoir plus sur System Manager link:view-insights.html["Informations"].
...
+
Sous *Unités de stockage*, une liste de toutes les unités de stockage dans la zone de disponibilité de stockage est affichée.



.Et la suite ?
Si votre zone de disponibilité de stockage manque d'espace, vous devriez prévoir de link:../manage-data/move-storage-units.html["déplacer les unités de stockage"] migrer vers une autre zone de disponibilité de stockage afin d'équilibrer l'utilisation du stockage sur l'ensemble du cluster, de supprimer des instantanés pour libérer de l'espace ou d'ajouter des disques supplémentaires à vos nœuds pour augmenter la capacité.
