...
Une unité de stockage ASA r2 a une capacité maximale de 128 To. Voir lelink:https://hwu.netapp.com/["NetApp Hardware Universe"^] pour connaître les limites de stockage les plus récentes pour les systèmes ASA r2.

Vous ajoutez et associez les initiateurs hôtes à l'unité de stockage dans le cadre du processus de création de l'unité de stockage.  Vous pouvez égalementlink:provision-san-storage.html#add-host-initiators["ajouter"] etlink:provision-san-storage.html#map-the-storage-unit-to-a-host["carte"] Les initiateurs hôtes après la création des unités de stockage.
...
link:../data-protection/snapshot-reserve.html["Découvrez plus d'informations sur la réserve de snapshots sur les systèmes ASA r2"].

Les unités de stockage sont configurées en allocation dynamique par défaut. L'allocation dynamique permet à l'unité de stockage d'atteindre la taille allouée sans réserver l'espace à l'avance. L'espace est alloué dynamiquement à partir de l'espace libre disponible, selon les besoins. Cela vous permet d'obtenir une plus grande efficacité de stockage en _surprovisionnant_ votre espace disponible. Par exemple, supposons que vous disposiez de 1 To d'espace libre et que vous deviez créer quatre unités de stockage de 1 To. Au lieu d'ajouter immédiatement 3 To de stockage supplémentaire à votre système, vous pouvez créer les unités de stockage, surveiller l'utilisation de l'espace et augmenter votre capacité de stockage au fur et à mesure que les unités de stockage consomment de l'espace réel. En savoir plus sur link:https://docs.netapp.com/us-en/ontap/concepts/thin-provisioning-concept.html["provisionnement fin"^].
...
CAUTION: Si vous créez plus d'une unité de stockage, chaque unité est créée avec la même capacité. Multipliez le nombre d'unités de stockage que vous créez par la capacité souhaitée pour vous assurer de disposer de suffisamment d'espace utilisable. Si vous n'avez pas assez d'espace libre et que vous choisissez de surprovisionner, surveillez attentivement l'utilisation afin d'éviter de manquer d'espace et de perdre des données.

+
À partir d'ONTAP 9.19.1, si vous créez un LUN AIX que vous prévoyez d'activer pour la protection SnapMirror active sync, sous *Informations sur l’hôte*, sélectionnez *local* pour le *Mode d’accès*. Ceci est nécessaire pour activer le basculement transparent des applications.
...
+
S'il n'existe pas de groupe d'hôtes préexistant pour les hôtes exécutant le système d'exploitation que vous avez spécifié, ONTAP crée automatiquement un nouveau groupe d'hôtes.
...
.. Définissez une limite de QoS, une garantie de QoS, ou les deux.
+
...
Les systèmes ASA r2 offrent deux niveaux de performances.  Le niveau de performance par défaut est *Extrême*, qui est le niveau le plus élevé disponible.  Vous pouvez abaisser le niveau à *Performance*.
 ...
 a| 
Modifiez la réserve de snapshots par défaut et activez la suppression automatique des snapshots.
 a| 
...
+
Formatez le NQN de l'hôte comme <nqn.yyyy-mm> suivi d'un nom de domaine pleinement qualifié.  L'année doit être égale ou postérieure à 1970.  La longueur maximale totale doit être de 223.  Un exemple d'initiateur NVMe valide est nqn.2014-08.com.example:string
