...
Si vous utilisez le protocole iSCSI ou FC et utilisez les outils ONTAP pour VMware Sphere, vous pouvez éventuellement link:https://docs.netapp.com/us-en/netapp-solutions/vmware/vmware-vmsc-with-smas.html["utilisez ONTAP Tools pour VMware pour configurer SnapMirror Active Sync"^] .


NOTE: À partir d'ONTAP 9.19.1 RC, la protection autonome contre les ransomwares avec intelligence artificielle (ARP/AI) prend en charge les unités de stockage principales en lecture-écriture dans les relations SAN synchronisées actives SnapMirror sur les systèmes ASA r2. ARP/AI s'exécute sur les unités de stockage principales actives, crée des instantanés ARP dédiés à ces unités et assure la détection des ransomwares par apprentissage automatique. Pour activer ARP/AI, consultez link:../secure-data/enable-anti-ransomware-protection.html["Activez la protection autonome contre les ransomwares grâce à l'IA"].
